...
function make(width, height, type) {
  ...
  for (let y = 0; y < height; y++)
    for (let x = 0; x < width; x++) {
      ...
      if (
        ((x === 6 || x === width - 7) && y >= 5 && y < height - 5) ||
        ((y === 5 || y === height - 6) && x >= 6 && x < width - 6)
      )
        pen = 2;
  ...
  return AmigaIcon.create({
    width,
    height,
    type,
    normalPixels: pixels,
    selectedPixels: pixels.map((p) => (p === 1 ? 2 : p === 2 ? 1 : p)),
  });
}
for (const [name, w, h, type] of [
  ...
  await writeFile(
    new URL(`../samples/${name}`, import.meta.url),
